...
echo "$EXPLANATORY This demo builds a swarm from VMs on this "
echo "$EXPLANATORY host utilizing docker-machine. If you "
echo "$EXPLANATORY don't have docker-machine and virtualbox "
echo "$EXPLANATORY installed, hit Ctrl+C now."
echo "$EXPLANATORY Assuming you have virtualbox installed, "
echo "$EXPLANATORY if you also have the GUI, you can pop it "
echo "$EXPLANATORY open now to watch the machines come up in "
echo "$EXPLANATORY real time in the GUI"
...
echo "$EXPLANATORY If you've had the virtualbox GUI open until now"
...
echo "$EXPLANATORY The orchestration repository requires the envsubst command, "
echo "$EXPLANATORY part of the GNU gettext package. This isn't installed by "
echo "$EXPLANATORY default in the boot2docker images, so lets grab it."
echo "$EXPLANATORY $ docker-machine ssh manager1 \"tce-load -wi gettext.tcz\""
echo "$EXPLANATORY Hit Enter to Execute the Command"
read
docker-machine ssh manager1 "tce-load -wi gettext.tcz"
echo "$EXPLANATORY gettext package installed on manager1!"
echo "$EXPLANATORY Hit Enter to Continue"
read
echo "$EXPLANATORY The docker build process also requires docker-compose,"
echo "$EXPLANATORY which isn't installed in boot2docker for some reason."
echo "$EXPLANATORY We need to grab the containized image of docker-compose "
echo "$EXPLANATORY and toss it in our manager path"
...
echo "$EXPLANATORY $ docker-machine ssh manager1 \"cd microservice_repository_orchestration && ./clone_repos.sh\""
echo "$EXPLANATORY Hit Enter to Execute the Command"
read
...
echo "$EXPLANATORY $ docker-machine ssh manager1 \"cd microservice_repository_orchestration && cat vars.env | sed 's/SWARM_HOST=.*$/SWARM_HOST=http:\/\/\$(docker-machine ip manager1)/' > vars.env\""
echo "$EXPLANATORY Hit Enter to Execute the Command"
read
...
echo "$EXPLANATORY $ docker-machine ssh manager1 \"cd microservice_repository_orchestration && ./build_swarm_yml.sh\""
echo "$EXPLANATORY Hit Enter to Execute the Command"
read
